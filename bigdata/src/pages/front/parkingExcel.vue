<template>
  <q-page class="row page fit " style="padding-top:80px">
    <div class="col-12 cust-col section-padding " style="padding:4vh">
      <q-table id='ex_table' class="full-width pop-box" ref="table" color='cyan-12' :dark='true' title='停车场数据导出' :data="serverData" :columns="currentExType.columns" :filter="filter" loading-label='正在加载' row-key="rowNum" :selected.sync="selectedSecond" :pagination.sync="serverPagination" :loading="loading" :separator="separator" @request="request">
        <template slot="top" slot-scope="props">        
          <div class="row full-width tb-title">
            <div class="col-2">             
              <q-select dark color='cyan-12' class="ex_title " borderless @input="onExTypeChange" v-model="exType" radio :options='exportTypes' />           
            </div>
          </div>
          <div class="row gutter-sm fit items-center">
            <div class="col-2 ">             
                <q-select outlined dense dark color='cyan-12' v-model="ScenicName" @input='onScenicNameChange' radio :options="ScenicNameOptions" />              
            </div>
            <div class="col-2 ">             
                <q-select outlined dense color='cyan-12' dark v-model="trackNo" @input="onTrackNoChange" radio :options="trackNoOptions" />             
            </div>
            <div class="col-7 ">
              <datepicker id='ex_tb' @query='dateChange' />
            </div>           
            <div class="col-1">             
                <q-btn type='a' :href="href" download='停车来源' :disable='!isCanExport' icon='bar_chart' color='blue' label='导出'></q-btn>            
              
            </div>
            <div v-if='exType.value==1' class="col-2 ">              
  ...
  data () {
    ...
    methods: {
      ...
      loadScenicList() {
        this.$axios({
          ...
          if (this.ScenicNameOptions.length > 0) {
            this.ScenicName = this.ScenicNameOptions[0]
          }
        }).catch()
      },
      ...
      request(props) {
        ...
        if (this.ScenicName && this.ScenicName.value != '高淳全域') {
          params.scenicName = this.ScenicName.value
          links += `&scenicName=${this.ScenicName.value}`
        }
        if (this.trackNo) {
          params.trackNo = this.trackNo.value
          links += `&trackNo=${this.trackNo.value}`
        }
        if (this.exType == 1) {
          params.province = this.province.value
          links += `&province=${this.province.value}`
        }
        this.href = links
        this.loading = true;
        this.$axios({
