<template>
  <q-carousel color="blue" class="round-border section-bg" quick-nav infinite :autoplay="autoplay" height="100%">
    <template v-if="platform.id=='10001'">
      <q-carousel-slide class="flex flex-center section-padding">
        <div class="column fit">
          <!-- <div class="col-6">
          <echart key="p1c31" :options='p1c31' />
        </div> -->
          <div class="col-12">
    ...
    <!-- <q-carousel-control slot="control-button" slot-scope="carousel" position="bottom-right" :offset="[18, 22]">
      <q-btn round dense push color="amber" :icon="carousel.inFullscreen ? 'fullscreen_exit' : 'fullscreen'" @click="carousel.toggleFullscreen()" />
    </q-carousel-control> -->
         ...
         this.$axios({
          ...
          if (res.desc&&res.desc.length>0) {            
            option.series[0].data=res.desc.map(m=>{
              ...
              option.series[0].data.push({name:date.formatDate(new Date(),'YYYY-MM-DD HH时'),value:0})
      ...
      loadCarSource() {
        ...
        this.$axios({
          ...
          if(res.desc.length>0){
            ...
            option.series[0].data=[{name:'江苏',value:0},{name:'安徽',value:0},{name:'其他',value:0}]
          }
           this.p1c1=option
      ...
      loadTicketPieChartStatistics(statisticsType) {
        ...
        let option = {
          ...
          series: [{
            name: '支付方式',
            type: 'pie',
            radius: statisticsType == 'pay' ? '25%' : ['20%', '30%'],
            center: ['50%', '50%'],
            data: [],
            label: {
              normal: {
